...
internal let demoPullbackReducer = Reducer<DemoPullbackState, DemoPullbackAction, DemoPullbackEnvironment> { state, action, env in
    switch action {
    case .didLoad:
        print("page didload, tracking with userId: ", env.userId())
        return .none
    case .productCard(.didTap):
        return env.route(state.productCard.url)
            .fireAndForget()
